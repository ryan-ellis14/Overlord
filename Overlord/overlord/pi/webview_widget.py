...
class WebviewWidget(Gtk.Bin):
    ...
    def __init__(self, url):
        ...
        self.show_all()
        logger.debug("WebviewWidget created for: %s", url)

    def _setup_user_agent(self):
        settings = self._webview.get_settings()
        ua = settings.get_user_agent()
        if ua and "Overlord" not in ua:
            settings.set_user_agent(ua + " Overlord/2.0-Pi")

    def _setup_content_manager(self):
        manager = self._webview.get_user_content_manager()

        manager.register_script_message_handler("overlord_log")

        self._webview.connect("user-message-received", self._on_user_message)
    ...
    def _on_load_changed(self, webview, event):
        if event == WebKit2.LoadEvent.FINISHED:
            logger.debug("Webview loaded: %s", self._url)

    def _on_user_message(self, webview, message):
        pass
